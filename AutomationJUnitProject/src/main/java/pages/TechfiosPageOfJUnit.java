...
public class TechfiosPageOfJUnit extends BasePageJUnit {
	...
	WebElement CATEGORY_TEXTBOX_ELEMENT;
	...
	public TechfiosPageOfJUnit(WebDriver driver) {
		this.driver = driver;
	}
	...
	public boolean validateOneCheckboxIsRemoved() {
	...
	public boolean validateAllCheckBoxesAreRemoved() {
	...
	public boolean addUniqueCategory(String categoryName) {
		...
		CATEGORY_TEXTBOX_ELEMENT.sendKeys(categoryName);
		try {
			Thread.sleep(3000);
		} catch (InterruptedException e) {
			e.printStackTrace();
	...
	public String addDuplicateCategoryName() {
		...
		CATEGORY_TEXTBOX_ELEMENT.sendKeys(duplicateName);
		try {
			Thread.sleep(3000);
		} catch (InterruptedException e) {
			e.printStackTrace();
